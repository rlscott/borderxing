...
Figure \@ref(fig:tsMexico) shows the number of crossings on the Mexico border over time. We utilize the 'additive' method to decompose the population data at the Mexico border, dividing it into three components: trend, season, and random parts. Our focus for this project lies in analyzing the seasonal component.

```{r tsCanada,fig.align='center',out.width="40%", echo=F, message=F, fig.cap="Time series data related to the value of US-Canada border."}
...
c_long%>%
  ...
  ggtitle("Comparision of Canada and Mexico time series")
```
...
p2 + geom_hline(aes(yintercept = wt),line2, color = 'darkolivegreen3')+
  ggtitle("September 11th")
```

Looking into to major events, as observed in the Figure \@ref(fig:impact911), 9/11 had a noticeable impact on the value of both borders. There was a significant decline in value on September 11th, 2001, followed by a subsequent recovery the following year.
...
We also compared the temporal trends of both people and objects crossing. First, to focus on people, we selected the Mexico and Canada border data, filtering by the 'Type' attribute equal to 'People.' From the data, we identified the 10 largest values and extracted their corresponding port name. The findings revealed that for the Mexico border, the predominant port is El Paso, while for the Canada border, the prominent ports are Buffalo Niagara Falls and Detroit. As these three ports exhibit significant interest, we will conduct further research on them.
...
```{r bar,fig.align='center',out.width="50%", echo=F, message=F, fig.cap="Comparison of object crossings"}
border1 %>%
