...
```{r all-map, fig.cap = "Map of the 118 US Border Ports of Entry", fig.align='center', out.width="70%"}
...
## Anomalies in the US-Canada ports of entry
...
# Seasonal and Inter-year Trends

We also aggregated Canadian Border data into seven-year intervals, examining the average transportation volumes for both objects and people in Figure \@ref(fig:avg-trans). Our analysis revealed a pronounced seasonal trend, with peak transportation occurring during the summer and a corresponding underestimation during the winter months.
...
avg_obj <- ggplot() + 
  ...
  scale_color_manual(name = "Year Group", values = c("1996-2002" = "deepskyblue1", "2003-2009" = "deepskyblue2","2010-2016" = "deepskyblue3", "2017-2023" = "deepskyblue4")) + ggtitle('Average of crossing in object within 7 years') + ylab('Numbers of Crossing in Object') + theme_bw()
...
avg_ppl <- ggplot() + 
  ...
  scale_color_manual(name = "Year Group", values = c("1996-2002" = "deepskyblue1", "2003-2009" = "deepskyblue2","2010-2016" = "deepskyblue3", "2017-2023" = "deepskyblue4")) + ggtitle('Average of crossing in people within 7 years')+ ylab('Numbers of Crossing in People') + theme_bw()

gridExtra::grid.arrange( avg_obj, avg_ppl, nrow = 2, ncol = 1)

```
```{r tsMexico,fig.align='center', out.width="40%",echo=F, message=F, fig.cap="Time series data related to the value of Mexico border.", warning= FALSE}
Mexico_value <-border %>%
...
Next we overlay Mexico and Canadas seasonal trends over each other to compare their pattern seen in Figure \@ref(fig:tscompare).

```{r tscompare,fig.align='center', out.width="50%",echo=F, message=F, fig.cap="Comparison between the seasonal components of two borders.", warning= FALSE}
year <- 0
start_date <- as.Date(paste(year, "-01-01", sep = ""))
end_date <- as.Date(paste(year, "-12-31", sep = ""))
dates <- seq(start_date, end_date, by = "month")
...
## Historical Anomalies Interacting with Aggregate Border Crossings
...
Returning to the original trends containing Calexico and El Paso, we can quantify the observed difference between the year before and the year after 2001, to get an idea of the effect on both borders.
...
Canada2000<-Canada_value %>%
  filter(year == 2000) 
c1<-mean(Canada2000$Value)
Canada2002<-Canada_value %>%
  filter(year == 2002) 
c2<-mean(Canada2002$Value)
...
line1 <- data.frame(Border = c("US-Canada Border","US-Mexico Border"),wt = c(c1,c3))
line2 <- data.frame(Border = c("US-Canada Border","US-Mexico Border"),wt = c(c2,c4))
p2 <- p1 + geom_hline(aes(yintercept = wt),line1, color = 'skyblue4')
p2 + geom_hline(aes(yintercept = wt),line2, color = 'darkolivegreen3')+
  ggtitle("September 11")
```

As observed in the Figure \@ref(fig:impact911), the 9/11 event had a noticeable impact on the value of both borders. There was a large decline in value on September 11th, 2001 with the Mexican border, followed by a subsequent partial recovery the following year. Canada's border has a less sharp decline, and instead seems to be trending downwards. We can do the same thing with Covid 19.
...
p3 <- border %>% 
  ...
  facet_wrap(~Border,scales = "free_y")+
  theme_bw()+
  xlab("Time")+ ylab("Number of Crossings") + 
  theme(legend.position = "none")

Canada2019<-Canada_value %>%
  filter(year == 2019) 
c5<-mean(Canada2019$Value)
Canada2021<-Canada_value %>%
  filter(year == 2021) 
c6<-mean(Canada2021$Value)
...
line3 <- data.frame(Border = c("US-Canada Border","US-Mexico Border"),wt = c(c5,c7))
line4 <- data.frame(Border = c("US-Canada Border","US-Mexico Border"),wt = c(c6,c8))
...
# Analyseing the Types of Crossings for the Largest Ports
...
border1 %>%
  ...
  theme_bw()+ ylab("Number of Crossings") + xlab("Port name") + 
...
# Scepticism & questions about the data
